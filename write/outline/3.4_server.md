...
- Each array element contains: `epc`, `rssiDbm`, `timestamp`, `timeBasis` (`synced` / `estimated` / `relative`)
...
**[Figure 3.4.2-1: MQTT ingestion sequence - firmware batch publish → broker → scan handler → per-element validation → DB insert → WebSocket broadcast of raw scan event]**
...
**[Figure 3.4.3-2: Cluster timeline diagram - horizontal time axis; two rows (Lighthouse A = outside, Lighthouse B = inside); scan detections shown as vertical ticks; cluster start/end markers; centroid markers for Algorithm 1 and Algorithm 2; gap between waves illustrating centroid separation]**
...
**[Figure 3.4.3-3: RSSI trend diagram for Algorithm 2 - two panels (outside lighthouse, inside lighthouse); scatter plot of RSSI vs. time for an IN traversal; regression lines shown; outside slope < 0, inside slope > 0; trend agreement illustrated]**
...
- Tag assignment via live RFID capture: dashboard sends `scan:capture` WebSocket message to put a specific lighthouse into capture mode; next tag detection on that lighthouse is returned directly to the client rather than entering the scan pipeline

**[Figure 3.4.4-1: User/tag data model - users table (id, syncId, label, email); tags table (epc, label); userTagAssignments junction (userId, epc, assignedAt, revokedAt); active assignment enforced by null revokedAt constraint]**

**[Figure 3.4.4-2: Live tag capture sequence - dashboard sends scan:capture → server sets lighthouse capture mode → firmware detects tag → server intercepts scan → WebSocket delivers EPC to dashboard → capture mode cleared]**
...
**[Figure 3.4.5-1: Navigo3 integration sequence diagram - new processed_event → eligibility check → immediate pushEvent/pushPair → on success: syncedToIntegration = true; on failure: left false → retry poller picks up on next cycle]**
