...
![Figure 3.4.3-1: Event sweeper cycle flowchart - tick -> query closed clusters -> for each cluster: fetch scans -> processCluster() -> on success: mark processedAt; on failure: orphan or skip based on reason and age](./images/3.4.3-1_event_sweeper.svg)
...
- `outsideCentroid < insideCentroid` -> **in**; `insideCentroid < outsideCentroid` -> **out**; |delta| <= 1 ms ->
  **unknown**
