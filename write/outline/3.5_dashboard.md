...
- Real-time updates are delivered via a WebSocket connection established on application mount (`App.tsx`); the WebSocket
  ...
  polling
<!-- - The API uses JWT authentication; dashboard users authenticate via `POST /api/v1/auth/login` and the token is attached -->
...
- Pending (unregistered) devices that connect to the MQTT broker appear in a separate section; the operator can claim a
  pending device by assigning it a name and and placement (inside/outside)
...
<!-- - Each group has internally configurable `activityTimeoutMs` (default 4 s) controlling how long after the last scan -->
<!--   the cluster is considered closed, and `orphanTimeoutMs` (default 8 ms) controlling when single-lighthouse scans -->
<!--   are orphaned -->
...
- Each user can have multiple EPC tags assigned at a time; any orphaned tags assignments are preserved with a
