...
- Styling uses CSS/SCSS modules scoped per component
...
**[Figure 3.5.2-1: Processed event detail modal - Overview tab showing direction, confidence, confidence factor bars, and companion algorithm link. Same screenshot as outline_plus 3.4, page 7 top panel]**

**[Figure 3.5.2-2: Processed event detail modal - Timeline tab showing scan timing histogram with centroid markers and RSSI-over-time scatter plot with regression lines. Same screenshot as outline_plus 3.4, page 6 bottom panel / page 10 top panel]**
...
- Tag assignment uses a live RFID capture workflow: the operator selects a lighthouse, the dashboard sends a `scan:capture` WebSocket message, the next tag detected on that lighthouse is returned to the browser and offered for assignment - the tag never enters the normal scan pipeline during capture
