...
export async function closeDatabase(timeout = 5000): Promise<void> {
  ...
  try {
    ...
    const timeoutPromise = new Promise<void>((_, reject) => {
      setTimeout(() => reject(new Error('Database shutdown timed out')), timeout);
    });

    // Race between graceful close and timeout
    await Promise.race([
      sql.end({ timeout: timeout / 1000 }),
      timeoutPromise,
    ]);
 ...
 */
export function getPoolStats(): { isInitialized: boolean; isShuttingDown: boolean } {
  return {
