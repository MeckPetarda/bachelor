...
export function prependEvent(event: Scan) {
  ...
  if (filters.lighthouseId !== undefined && event.lighthouseId !== filters.lighthouseId) {
    return;
  }
  if (filters.epc && !event.epc.toLowerCase().includes(filters.epc.toLowerCase())) {
    return;
  }
  ...
  setState('events', (events) => [event, ...events.slice(0, state.pagination.limit - 1)]);
