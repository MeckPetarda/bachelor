 ...
 */
static void rfid_event_to_offline_event(
    const rfid_tag_event_t* rfid_event,
...
)
{
    ...
    memcpy(offline_event->epc, rfid_event->epc,
           rfid_event->epc_len < 24 ? rfid_event->epc_len : 24);
 ...
 */
static void offline_event_to_rfid_event(
    const offline_event_t* offline_event,
        ...
        if (xQueueReceive(logger_state.event_queue, &rfid_event, portMAX_DELAY)) {
            ...
            if (xSemaphoreTake(logger_state.storage_mutex, pdMS_TO_TICKS(1000))) {
                ...
                if (ret == ESP_OK) {
                    ...
                    ESP_LOGD(TAG, "Event stored at index %lu (pending: %lu)",
                            old_write_index, get_pending_count_internal());
                } else {
...
static void replay_task(void* arg)
{
            ...
            if (ret == ESP_OK) {
                ...
                if (validate_event_crc(&offline_event)) {
                    ...
                    if (logger_state.replay_callback) {
                        ...
                        ret = logger_state.replay_callback(
                            &rfid_event,
                            offline_event.timestamp_ms,
                            replay_time
                        );
                        ...
                        if (ret == ESP_OK) {
                            ...
                            ESP_LOGI(TAG, "Replayed event %lu/%lu",
                                    events_replayed, events_replayed + get_pending_count_internal());
                        } else {
    ...
    ESP_LOGI(TAG, "Replay complete! Replayed %lu events in %llu ms",
            events_replayed, replay_duration);
...
esp_err_t offline_logger_init(void)
{
    ...
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Partition size: %u KB, used: %u KB (%u%% free)",
    ...
    if (!rtc_initialized) {
        ...
        ESP_LOGI(TAG, "RTC pointers restored: write=%lu, read=%lu",
...
)
{
    ...
    BaseType_t ret = xTaskCreate(
        ...
        "offline_replay",
        REPLAY_TASK_STACK,
        (void*)(uintptr_t)grace_period_s,
        REPLAY_TASK_PRIORITY,
        &logger_state.replay_task
    );
